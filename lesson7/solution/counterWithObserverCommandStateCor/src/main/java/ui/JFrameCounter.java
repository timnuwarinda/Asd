...
import service.DecrementCommand;
import service.HistoryList;
import service.IncrementCommand;
...
public class JFrameCounter extends JFrame {
	...
	public JFrameCounter() {
		try {
			...
			counterService.addObserver(textframe);
			counterService.addObserver(rectframe);
			counterService.addObserver(ovalframe);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
